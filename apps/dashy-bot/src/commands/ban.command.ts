...
export const legacyUnbanCommand = new CommandInteractionConstructor(
    'legacyban',
    async (client, message, args) => {
        if (!(await checkOwner(message, client))) {
            return {
        ...
        if (!args[0]) {
            return {
                status: 400 as statusResponse,
                data: 'You must pass ID of user to unban.',
            };
        }
        return guildedRestService
            .unbanMember(message.serverId, args[0])
            .then(() => {
                return {
                    status: 200 as statusResponse,
                    data: `Successfully unbanned: ${args[0]}`,
                };
            })
            .catch((err) => {
                if (err.code === 400) {
